...
# Read a setting value from settings.conf without sourcing the file.
...
get_setting_value() {
    ...
    raw_value=$(grep -E "^${key}=" "$file" | tail -n 1 | cut -d'=' -f2-)
    raw_value="${raw_value%\"}"
    raw_value="${raw_value#\"}"
    raw_value="${raw_value%\'}"
    raw_value="${raw_value#\'}"
    ...
    while true; do
        ...
        case $main_choice in
            ...
            3)
                # Load settings
                if [[ -f "${BASEDIR}/settings.conf" ]]; then
                    com_port=$(get_setting_value "com_port" "${BASEDIR}/settings.conf")
                    com_port_settings=$(get_setting_value "com_port_settings" "${BASEDIR}/settings.conf")
                    receiver=$(get_setting_value "receiver" "${BASEDIR}/settings.conf")
